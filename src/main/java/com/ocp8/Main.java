...
//主程式
    ...
    public static void main(String[] args) {
        ...
        for (Transportation ts : trans) {
            if (ts instanceof Car) { // instanceof只挑陣內的其中一種項目
                ((Car) ts).play(); // ts原為Transportation的類別，所以要挑car 需要轉型為car，才能只印出car
        ...
        // 試問總租金?
          int sum = 0;
            ...
            for (Transportation transportations : trans) {
                System.out.println();
                sum += transportations.getPrice();  
        }
System.out.println("租金:  " + subtotal);
    }
